...
function completeOJT($ID) {
    global $connection;
    $sql_update = "UPDATE user_data SET OJT='Done' WHERE ID='$ID'";
    if (mysqli_query($connection, $sql_update)) {
        echo "";
    } else {
...
$sql_approved = "SELECT *, IFNULL(locked, 0) AS locked FROM user_data WHERE STATUS = 'approve' AND OJT != 'Done'"; // Excluding users with OJT status set to "Done"
...
while($row = $result_approved->fetch_assoc()){
    ...
    <tr>
        ...
        <td>
            ...
            <a href='dashboard.php?ID={$row["ID"]}&action=completeOJT'><i class='fas fa-check'></i></a> <!-- Icon to mark OJT as Done -->
        </td>
    </tr>
    ";
}
