...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['message'])) {
    ...
    $check_receiver_sql = "";
    if ($receiver_id == 'all') {
        $check_receiver_sql = "SELECT ID FROM user_data";
    } else {
    ...
    if ($check_sender_result->num_rows == 1 && $check_receiver_result->num_rows > 0) {
        if ($receiver_id == 'all') {
            while ($receiver_row = $check_receiver_result->fetch_assoc()) {
...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['receiver_id'])) {
    ...
    $message_sql = "";
    if ($selected_receiver_id == 'all') {
        $message_sql = "SELECT m.message, m.timestamp, u1.NAME AS sender, 'All Users' AS receiver, m.sender_id
                        FROM messages m
                        JOIN (SELECT ID, NAME FROM admin UNION SELECT ID, NAME FROM user_data) u1 ON m.sender_id = u1.ID
                        WHERE m.receiver_id = 'all'
    ...
    <a href="?receiver_id=<?php echo $row['ID']; ?>">
        ...
        <?php echo htmlspecialchars($row['ID']) . ' - ' . htmlspecialchars($first_name); ?>
            ...
            <?php endwhile; ?>
                        ...
                        <a href="?receiver_id=all">Send Message to All Users</a>
                    </ul>
                </div>
            </div>
